...
Reset only this lab (destructive for uncommitted edits in this path): first inspect `git diff -- labs/5.5/5.5-lab`, then run `git restore --source=HEAD -- labs/5.5/5.5-lab` only if you intend to discard those edits. Never use a repository-wide reset or restore.
