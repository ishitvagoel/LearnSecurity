...
// Keep the vocabulary used in design reviews and standards, then define it in
// place for readers who are meeting the term for the first time.
const PROSE_PHRASES: [RegExp, string][] = [
  [/\btrusted computing base\b/gi, "trusted computing base (what you trust)"],
  [/\bincomplete mediation\b/gi, "incomplete mediation (a skipped check)"],
  [/\bcomplete mediation\b/gi, "complete mediation (checking every path)"],
  [/\bfail-safe defaults\b/gi, "fail-safe defaults (failing closed)"],
  [/\bleast privilege\b/gi, "least privilege (only the permission that is needed)"],
  [/\bseparation of privilege\b/gi, "separation of privilege (two independent checks)"],
  ...
  [/\binvariant catalogues?\b/gi, "__INVARIANT_CATALOGUE__"],
  [/\ban invariant\b/gi, "an invariant (a rule that must stay true)"],
  [/\binvariants\b(?!\s*\()/gi, "invariants (rules that must stay true)"],
  [/\binvariant\b(?!\s*\()/gi, "invariant (a rule that must stay true)"],
  [/__INVARIANT_CATALOGUE__/g, "invariant catalogue (a list of rules)"],
  ...
  [/\battack surfaces\b(?!\s*\()/gi, "attack surfaces (ways in)"],
  [/\battack surface\b(?!\s*\()/gi, "attack surface (ways in)"],
  [/\bTop 10\b/g, "a famous-bugs list"],
  [/\bawareness lists?\b/gi, "famous-bugs lists"],
  [/\bthis origin\b/gi, "this website"],
  [/\bthe TCB\b(?!\s*\()/g, "the TCB (what you trust)"],
  [/\bTCB\b(?!\s*\()/g, "TCB (what you trust)"],
  ...
  [/Do not claim the verification gate\./g, "This page does not finish a check-in."],
  [/Do not claim the ship gate\./g, "This page does not finish the ship check-in."],
  ...
  [/\bOWASP ASVS 5\.0\.0 \(final\)/g, "OWASP ASVS 5.0.0 (the published web-security checklist)"],
  [/\bASVS 5\.0\.0\b(?!\s*\()/g, "ASVS 5.0.0 (the published web-security checklist)"],
  [/\bASVS\b(?!\s*\d|\s*\()/g, "ASVS (the published checklist)"],
  [/\bMASVS\b(?!\s*\()/g, "MASVS (the phone-app checklist)"],
  [/\bWSTG\b(?!\s*\()/g, "WSTG (the testing guide)"],
  ...
  [/\bthe verification gate\b/g, "this check-in"],
  [/\bthe ship gate\b/g, "the ship check"],
  [
