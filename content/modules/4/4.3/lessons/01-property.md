...
FastAPI will bind query params. Next.js router will put them in the address bar. TLS encrypts the hop, not the log. What this practice is supposed to show: a query-only request yields `None`; cookie and header still work. Practice files are in `labs/4.3/4.3-lab`. No live CDNs.
