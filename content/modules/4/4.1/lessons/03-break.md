...
The practice is not a website you attack. It is a tiny Python `delete_user` and `session_valid`. The failure is already in the functions: delete marks the profile and leaves the session. Watch the check treat that leftover as a **failed rule**, not as a cleanup nit.
