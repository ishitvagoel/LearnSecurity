...
Industry lists ask for the check on a trusted server, not in the Next.js client. The check below is that sentence for `can_read`. Extra rows about applying grant changes immediately, and carrying the original person through a worker, are advanced — not this week's check.
