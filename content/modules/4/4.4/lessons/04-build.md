...
The check belongs on a trusted server, not in the Next.js client — `can_read`. Extra rows about applying grant changes immediately, and carrying the original person through a worker, are advanced — not this week's check.
