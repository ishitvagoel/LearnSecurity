# 7.1-LO-01 — API contracts, protocols, and inventory: property vs mechanism
...
What must remain true of **SecureCollab** (or the elective system) regarding **API contracts, protocols, and inventory** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For API contracts, protocols, and inventory |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **API contracts, protocols, and inventory**. Name what the app must still enforce.
...
Complete the associated lab under `labs/7.1/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **contracts** property were false.
...
- API contracts, protocols, and inventory is a Top 10 memorization exercise
