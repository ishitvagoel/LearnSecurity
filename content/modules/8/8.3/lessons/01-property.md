# 8.3-LO-01 — Network, deep links, WebViews, and inter-app communication: property vs mechanism
...
What must remain true of **SecureCollab** (or the elective system) regarding **Network, deep links, WebViews, and inter-app communication** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For Network, deep links, WebViews, and inter-app communication |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **Network, deep links, WebViews, and inter-app communication**. Name what the app must still enforce.
...
Complete the associated lab under `labs/8.3/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **network** property were false.
...
- Network, deep links, WebViews, and inter-app communication is a Top 10 memorization exercise
