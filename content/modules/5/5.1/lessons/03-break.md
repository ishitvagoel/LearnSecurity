...
The practice is not a warehouse you attack. It is a tiny Python `delete_account` plus `body_retained` / `search_retained`. The failure is already in the functions: delete pops the notes map and leaves analytics. Watch the check treat that leftover as a **failed rule**, not as a cleanup nit.
