...
The practice is not a website you attack. It is a tiny Python `close_incident` that returns true for every dict. The failure is already in the function: it never looks at recovery or logs. The point is to see that the check treats that always-true close as a **failed rule**, not as a paperwork nit.
