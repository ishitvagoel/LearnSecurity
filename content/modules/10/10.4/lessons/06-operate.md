...
A canary dashboard will show rollout percent and stay silent when CI’s `boot_ok` is always true. Detection must observe **prod plus debug is deny**, not “the container started.” A stack trace or a session token next to that prod-debug deny is a log-line leak.
