# 10.3-LO-01 — Cloud, serverless, containers, Kubernetes, and IaC: property vs mechanism
...
What must remain true of **SecureCollab** (or the elective system) regarding **Cloud, serverless, containers, Kubernetes, and IaC** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For Cloud, serverless, containers, Kubernetes, and IaC |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **Cloud, serverless, containers, Kubernetes, and IaC**. Name what the app must still enforce.
...
Complete the associated lab under `labs/10.3/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **cloud** property were false.
...
- Cloud, serverless, containers, Kubernetes, and IaC is a Top 10 memorization exercise
