...
GitHub’s audit log is not this lab’s trusted core. Process scores on a maturity dashboard stay green while CI’s `merge_ok` is always true. Detection must observe **empty change is deny**, not poster counts. A GitHub token is a secrets hole (5.3). A threat-model body copies the model into the ticket.
