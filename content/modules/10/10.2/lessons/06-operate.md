...
An npm audit dashboard will show advisory counts and stay silent when CI’s `install_ok` is always true. Detection must observe **aaa vs bbb is deny**, not CVE volume. Quote aaa vs bbb, not a registry token — that is the same leak as a log line (5.3).
