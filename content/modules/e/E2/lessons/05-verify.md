...
Open `labs/E2/e2-lab/tests/test_property.py`. Report-Only counted as on is the fail for `test_report_only_is_not_enforcement`.
...
An enforcing content-security policy can still look fine. Deny Report-Only treated as on. If the broken files do not fail `test_report_only_is_not_enforcement`, the practice is miswired — fix the wiring, not the check.
