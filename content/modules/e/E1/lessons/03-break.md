...
The practice is not a model you attack. It is a tiny Python `run_tool` that returns `ran {name}` for every name. The failure is already in the function: it never looks at the name. The point is to see that the check treats that always-run call as a **failed rule**, not as a paperwork nit.
