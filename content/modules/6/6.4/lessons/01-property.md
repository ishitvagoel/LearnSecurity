# 6.4-LO-01 — Files, paths, uploads, archives, XML, and deserialization: property vs mechanism
...
What must remain true of **SecureCollab** (or the elective system) regarding **Files, paths, uploads, archives, XML, and deserialization** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For Files, paths, uploads, archives, XML, and deserialization |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **Files, paths, uploads, archives, XML, and deserialization**. Name what the app must still enforce.
...
Complete the associated lab under `labs/6.4/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **files** property were false.
...
- Files, paths, uploads, archives, XML, and deserialization is a Top 10 memorization exercise
