...
Starlette `UploadFile.filename` is hostile. FastAPI does not canonicalize for you. Content-Type is a client claim. What this practice is supposed to show: after join and canonicalize, the object is still `/tmp/sc-lab` or a child. Practice files are in `labs/6.4/6.4-lab`. No live walk against a public upload folder.
