...
A network filter will page on cross-site POST volume and stay silent when `/share.json` still keys only the cookie. Notice must observe **origin mismatch or missing token at `allow_share`**, not CORS error counts. A session cookie or CSRF token on `allow_share` is a logging hole from an earlier topic.
