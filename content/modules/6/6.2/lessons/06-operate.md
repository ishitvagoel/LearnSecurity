...
A markdown path can bring raw HTML back. Patient titles and nicknames do not go in the ticket.
...
A content-security report-only dashboard will page on blocked scripts and stay silent when the stored title still contains raw `<`. Notice must observe **raw angle brackets at the encode sink**, not report-only counts. The title text on the encode-sink metric is a logging leak (3.1).
...
The title text, a note body, or an attack cookbook would turn the log into a cookbook.
