...
FastAPI does not scope PostgreSQL. Splitting into microservices without new grants is a topology drawing. What this practice is supposed to show: `can_select("app", "tB", "tA") is False` and the runtime connection is not `postgres`. Practice files are in `labs/3.3/3.3-lab`. No live databases.
