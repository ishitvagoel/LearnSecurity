...
- [ ] Transfer task (items.md #8): clinic REST/GraphQL scenario, naming which of C1–C5 change and which do not, with the GraphQL alias mechanism explained
...
- [ ] `vulnerable/` tests: 4 of 6 fail for the stated security reason; `fixed/` tests: 6 of 6 pass (authorized local practice files only)
...
| Transfer-ready | Item 8 done: the GraphQL alias mechanism explained in the learner's own words, connected explicitly to the "check every occurrence" principle from Lesson 03 |

Knowledge-check items (retryable) are items 1–6 in `items.md`; design, transfer, and operate items (7–9) require satisfactory evidence, not a retry-to-80% threshold — a critical gap here (for example, a fix that only handles the two-key case) is not compensated by strong answers elsewhere.
