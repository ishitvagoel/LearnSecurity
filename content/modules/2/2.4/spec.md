...
| C4 | 4 Build, 5 Verify, 7 Generalize, 8 Review | Not one dedicated assertion — exercised by the full vulnerable/fixed contrast (every C2/C3 test the fixed variant passes depends on the `UNIQUE` constraint being the mechanism) and by `08-review`'s seeded finding that `candidate_fix.py`'s per-call `threading.Lock()` is a failed attempt at exactly this claim. The lab's concurrency test is single-process (a `ThreadPoolExecutor` inside one `impl.app`), so it can only verify the within-process half of C4; a correctly module-level-shared lock would also pass it. The cross-process half — a shared lock does nothing for two separate `uvicorn` workers — is honestly declared verified by argument in `05-verify.md`, not by this suite, the same pattern C5 uses. | items.md #4, #5, #6 |
| C5 | 1 Property, 2 Model, 5 Verify, 7 Generalize | Not directly code-testable — no queue or worker exists in this Tier-2 fixture. Modeled in `lessons/01-property.md`'s closing residual, `lessons/02-model.md`'s "What can still go wrong," and `lessons/05-verify.md`'s "What a fully green suite still does not prove." | items.md #8 |
...
| Treat OWASP Top 10:2025 A10 as a regression check applied after the causal fix, not as the lesson's own structure | C3 | `lessons/06-operate.md` §Signal design | `lessons/06-operate.md` fail-closed operate example | `lessons/08-review.md` four-question checklist | Not directly item-tested — checked only by `08-review.md`'s seeded review and the rubric's transfer-ready band, neither of which is `items.md`; no item in the bank addresses A10-as-regression | `lessons/07-transfer.md` |
| Transfer the retry, race, and fail-closed reasoning to a different limited resource and state which claims survive the change unchanged and which do not | C1, C3, C4 | `lessons/07-transfer.md` §Which claims survive unchanged, and why | `lessons/07-transfer.md` naive count-check trace | `lessons/07-transfer.md` write-up prompts | items.md #6, #7 | (is the transfer task) |
