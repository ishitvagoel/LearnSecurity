...
Draw the map so someone else could name the checks without opening the keys file. Open `cache.py` in `labs/2.2/2.2-request-path`. Label path-only versus `(path, bound company)`.
