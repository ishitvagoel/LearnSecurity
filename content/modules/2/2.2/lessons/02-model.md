...
Draw the map so someone else could name the checks without opening the keys file. Look in `labs/2.2/2.2-request-path`, starting with `cache.py`. Label path-only versus `(path, bound company)`.
