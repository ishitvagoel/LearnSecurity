...
`labs/2.2/2.2-request-path/README.md` documents six constructed fakes verified against this suite before this module trusted it: a cache-only fix that leaves the header override open (caught by C2's forbidden-outcome test, its unregistered-key case, and its anti-fake test, three failures; 12 of 15 still pass); a header-only fix that leaves the cache keyed on path alone (caught symmetrically by C1's own forbidden-outcome and anti-fake tests, two failures; 13 of 15 still pass); a hostname check using `.startswith()` (caught by exactly one anti-fake test, the prefix one; 14 of 15 still pass); a hostname check using `in` (caught by both hostname anti-fake tests, since a substring match is a superset of a prefix match; 13 of 15 still pass); a trust-state check that reads "fails closed on an unrecognized state" as "checks only for `None`" — `if cert_trusted_ca is None: return False` in place of `if cert_trusted_ca is not True: return False` — which still fails closed on the unknown case but wrongly treats an explicitly-resolved `False` (a chain check that completed and failed) the same as `True` once hostname and version pass (14 of 15 still pass, and only the test built specifically to isolate that one signal catches it); and a version check that reads "only 1.2 and 1.3 are accepted" as a lexicographic lower bound (`tls_version < "1.2"`) instead of exact set membership, which wrongly accepts any version string that merely sorts above "1.2" as text — such as "1.9" — without being a member of the accepted set at all (14 of 15 still pass, caught only by the test built to isolate that signal). Each of these six fakes would look complete to a reviewer who only read the diff and not the tests — every one changes real code in the right direction, and every one still leaves a specific, describable input that reaches the forbidden outcome. The trust-state fake is also the reason this page does not claim "unknown trust state fails closed" and "untrusted CA fails closed" are the same test: a check that never completed and a check that completed and failed are different facts about the world, and a suite that only exercises the first has not verified the second. The version-check fake is the same lesson again, one signal over: "not below the oldest accepted version" and "a member of the accepted set" are different claims, and a suite that only ever sends versions on one side of that gap has not verified the second either.
