...
```mermaid
flowchart LR
  Run["pytest --impl vulnerable"] -->|"6 of 19 pass"| V1["C1 forbidden outcome: FAIL\n(cache leak reproduced)"]
  ...
  Fixed["pytest --impl fixed"] -->|"19 of 19 pass"| F1["C1, C2, C3 all hold,\nincluding every anti-fake test"]
...
`labs/2.2/2.2-request-path/README.md` documents ten constructed fakes verified against this suite before this module trusted it, each count checked by reconstructing the exact fake and running the real suite rather than incrementing an old number. A cache-only fix that leaves the header override open, matching `vulnerable/app.py`'s own check order — the API key's validity is decided first, then the header — so the malformed-credential case (an unregistered key with a company header) still passes; only the C2 forbidden-outcome test and its anti-fake pair catch it (two failures; 17 of 19 still pass). A header-only fix that leaves the cache keyed on path alone (caught symmetrically by C1's own forbidden-outcome and anti-fake tests, two failures; 17 of 19 still pass). A hostname check using `.startswith()` (caught by exactly one anti-fake test, the prefix one; 18 of 19 still pass). A hostname check using `in` (`expected_hostname in presented_hostname`) — caught by all three hostname anti-fake tests, not only two, because "origin.internal" is a substring of "evil-origin.internal" exactly as it is a substring of a prefix or middle-placed attacker string, so a check that only asks "does the expected name appear anywhere" cannot tell any of the three shapes apart (three failures; 16 of 19 still pass). A hostname check using `.endswith()` — the same mistake from the third boundary position, a name glued onto the end with no separator — caught by exactly one anti-fake test, the one built for this position (18 of 19 still pass). A trust-state check that reads "fails closed on an unrecognized state" as "checks only for `None`" — `if cert_trusted_ca is None: return False` in place of `if cert_trusted_ca is not True: return False` — which still fails closed on the unknown case but wrongly treats an explicitly-resolved `False` (a chain check that completed and failed) the same as `True` once hostname and version pass (18 of 19 still pass, and only the test built specifically to isolate that one signal catches it). A version check that reads "only 1.2 and 1.3 are accepted" as a lexicographic lower bound (`tls_version < "1.2"`) instead of exact set membership — caught by all FOUR version anti-fake tests, not only one: "1.9" sorts above "1.2" as text (the gap that first test was built for), but so does "1.4" — `"1.4" < "1.2"` is `False` in Python, since `"4" > "2"` character-by-character — so does "1.30", since the digit-by-digit comparison never reaches a character where they differ before running out, and so does "1.2\n", since a string that shares every character "1.2" has and then adds more is never lexicographically smaller than "1.2" itself, so this same fake also wrongly accepts the forward-compatible, parses-in-range, and incidental-whitespace tests' input, for three further, unrelated reasons (four failures; 15 of 19 still pass). A version check that reads the same requirement as "add the next plausible version to the allow-list," hand-writing `{"1.2", "1.3", "1.4"}` in place of `_ACCEPTED_TLS_VERSIONS` itself — "1.4" was never an accepted version in this fixture, but nothing else in the suite ever sends it (18 of 19 still pass, caught only by the test built specifically because the fakes before it never tried a version outside `{"1.1", "1.2", "1.3", "1.9"}`). A version check that reads "1.2 and 1.3 are accepted" as a numeric interval — parsing `tls_version` to a `float` and checking `1.2 <= version_num <= 1.3` instead of exact membership in the two-element string set, a natural instinct for anyone used to comparing version numbers arithmetically rather than as opaque strings — caught by both the parses-in-range anti-fake test it was originally built against and the incidental-whitespace anti-fake test, not only the first: `float("1.30") == 1.3` places "1.30" inside the numeric range even though it is not one of the two accepted strings, and `float("1.2\n")` is also `1.2`, because Python's `float()` strips incidental whitespace before parsing — a second, independent reason for the identical mistake (two failures; 17 of 19 still pass). And a version check that strips incidental whitespace before comparing — `tls_version.strip() not in _ACCEPTED_TLS_VERSIONS` in place of the bare membership test, a defensive habit many header-adjacent codebases apply reflexively to any string that arrived over the wire — passes 18 of 19 and is caught only by the test built to isolate this signal, since `"1.2\n".strip() == "1.2"` is a member of the accepted set even though `"1.2\n"` itself is not. Each of these ten fakes would look complete to a reviewer who only read the diff and not the tests — every one changes real code in the right direction, and every one still leaves a specific, describable input that reaches the forbidden outcome. The three-hostname-shape sequence (prefix, substring-anywhere, suffix-without-boundary) is the reason this page does not claim two hostname anti-fake tests are enough: each shape is a distinct fake that the other two do not catch, and the `in`-substring fake's failure against all three, discovered only once the third test existed, is why every count above was rebuilt from scratch rather than assumed stable across additions. The trust-state fake is also the reason this page does not claim "unknown trust state fails closed" and "untrusted CA fails closed" are the same test: a check that never completed and a check that completed and failed are different facts about the world, and a suite that only exercises the first has not verified the second. The four version-check fakes are the same lesson again, one signal over, four times: "not below the oldest accepted version," "a member of the accepted set," "numerically within the accepted range," and "the same string after discarding incidental whitespace" are four different claims, not one claim stated four ways, and the lexicographic fake's failure against all four anti-fake tests — not just the one it was originally built against, and growing by one every time a new version anti-fake test was added — is the same lesson the `in`-substring fake already taught about hostnames: a fake's failure count against the CURRENT suite, not the suite that existed when it was first isolated, is the only count worth trusting.
...
Run both variants and, for the thirteen tests that fail on `vulnerable`, name which row of the table above each one evidences:
