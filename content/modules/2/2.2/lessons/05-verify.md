...
Run both variants and, for the thirty-three tests that fail on `vulnerable`, name which row of the table above each one evidences (the eighteen parametrized near-miss-sweep cases all evidence the same row — treat them as one piece of evidence repeated eighteen ways, not eighteen independent findings):
