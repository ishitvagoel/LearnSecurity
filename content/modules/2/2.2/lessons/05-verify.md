...
```mermaid
flowchart LR
  Run["pytest --impl vulnerable"] -->|"6 of 37 pass"| V1["C1 forbidden outcome: FAIL\n(cache leak reproduced)"]
  ...
  Fixed["pytest --impl fixed"] -->|"37 of 37 pass"| F1["C1, C2, C3 all hold,\nincluding every anti-fake test"]
...
`labs/2.2/2.2-request-path/README.md` documents twelve constructed fakes verified against this suite before this module trusted it, each count checked by reconstructing the exact fake and running the real suite rather than incrementing an old number. A cache-only fix that leaves the header override open, matching `vulnerable/app.py`'s own check order — the API key's validity is decided first, then the header — so the malformed-credential case (an unregistered key with a company header) still passes; only the C2 forbidden-outcome test and its anti-fake pair catch it (two failures; 35 of 37 still pass). A header-only fix that leaves the cache keyed on path alone (caught symmetrically by C1's own forbidden-outcome and anti-fake tests, two failures; 35 of 37 still pass). A hostname check using `.startswith()` (caught by exactly one anti-fake test, the prefix one; 36 of 37 still pass). A hostname check using `in` (`expected_hostname in presented_hostname`) — caught by all three ASCII hostname anti-fake tests, not only two (three failures; 34 of 37 still pass). A hostname check using `.endswith()` — the same mistake from the third boundary position — caught by exactly one anti-fake test (36 of 37 still pass). A trust-state check using `is None` in place of `is not True` (36 of 37 still pass, caught only by the explicitly-untrusted-CA test). A version check that reads "only 1.2 and 1.3 are accepted" as a lexicographic lower bound (`tls_version < "1.2"`) — caught by all four dedicated version anti-fake tests plus twelve of the seventeen near-miss sweep cases, including the sweep's non-ASCII case: a fullwidth `"１.２"` sorts by Unicode code point above ASCII `"1.2"` exactly as `"1.9"` already did (sixteen failures; 21 of 37 still pass). A version check that reads the same requirement as "add the next plausible version to the allow-list," hand-writing `{"1.2", "1.3", "1.4"}` — passes 36 of 37, caught only by its own dedicated test, since none of the sweep's seventeen strings is `"1.4"`. A version check that reads "1.2 and 1.3 are accepted" as a numeric interval, parsing `tls_version` to a `float` — caught by both dedicated tests plus twelve sweep cases, including the fullwidth one, since Python's `float()` accepts fullwidth Unicode digits and parses `"１.２"` to `1.2` (fourteen failures; 23 of 37 still pass). And a version check that strips incidental whitespace before comparing — caught by its one dedicated test plus four sweep cases placing whitespace in a position that test's single case never covered, but NOT the fullwidth case, since a fullwidth digit is not whitespace and `.strip()` leaves it untouched (six failures; 31 of 37 still pass).

The eleventh fake, found in a sixth independent review round, was the first one this module's sweep test caught on its own before a dedicated test for it ever existed — and re-deriving its count against the CURRENT suite, rather than trusting the count recorded when it closed, shows it fails a third sweep case beyond the two the sixth review named. A sixth review round constructed an `.isdigit()`-guarded `(major, minor)` integer-tuple parse, deliberately hardened past the whitespace-stripping fake above (`"1.2\n".isdigit()` is correctly `False`) — but `int("01")` and `int("02")` both discard a leading zero, so `"01.2"` and `"1.02"` both parse to `(1, 2)` and are wrongly accepted; `"1.02"` was added as a sixteenth sweep case at the time to cover both of that review's named examples. It ALSO wrongly accepts the fullwidth case added later for an unrelated finding, since `str.isdigit()` returns `True` for fullwidth Unicode digits too, and `int()` parses them the same as their ASCII counterparts. The fake now fails exactly these three sweep cases and none of the (now nine) dedicated anti-fake tests (three failures; 34 of 37 still pass) — the same recompute-don't-increment discipline this module has needed at every round.

The twelfth fake, found in a seventh independent review round, is a different technique from all eleven before it — Unicode canonical-compatibility normalization before comparing, rather than parsing or partial matching — and the first fake to affect both the version AND the hostname check with the same one-line change to each. `unicodedata.normalize("NFKC", ...)` before the version-membership check and before the hostname `==` is a defensive habit standard security guidance actively recommends for untrusted strings in general. Isolated to the version check, it fails only the sweep's fullwidth case (one failure; 36 of 37 pass); isolated to the hostname check, it fails only the new dedicated Unicode-confusable hostname test (one failure; 36 of 37 pass), since none of the three ASCII hostname anti-fake tests uses a non-ASCII character; combined, as a reviewer would plausibly write it, it fails exactly these two tests and no others (two failures; 35 of 37 still pass).

None of these twelve fakes was constructed in response to the sweep test; the ten found before it existed were closed one at a time by a human reviewer across five independent review rounds, the eleventh was found in a sixth round after the sweep already existed and caught on first contact, and the twelfth was found in a seventh round and closed by extending the sweep with one more case plus one new dedicated hostname test. `test_anti_fake_version_near_miss_sweep_is_rejected` asserts rejection across seventeen systematically-chosen near-misses in one test — whitespace in positions the dedicated tests do not cover, leading and trailing zeros in either version position, an extra version segment, a sign prefix, exponential notation, an accepted-looking substring embedded in a longer string, and a non-ASCII digit sequence — and, as the counts above show, independently rediscovers three of the four already-closed version fakes from a different angle, without having been told which fakes those were, plus the fifth and sixth (the eleventh and twelfth fakes above) on first contact for at least part of each. Two further fakes, never constructed by any review round, confirm the sweep catches shapes nobody had isolated by hand yet: a regex check (`re.search(r"1\.[23]", tls_version)`) would have passed every test that existed before the sweep but fails all fifteen ASCII-only sweep cases — not the leading-zero or fullwidth ones, since neither contains the literal substring "1.2" or "1.3" — plus two of the dedicated tests (seventeen failures total); and an `(major, minor)` integer-tuple parse with no digit-only guard at all fails ten sweep cases (the leading-zero pair and the fullwidth case, since bare `int()` accepts fullwidth digits too) plus one dedicated test (eleven failures total). Neither of these two counts toward the twelve fakes above, because no review round ever constructed either one; they exist only to show the sweep's value was never limited to the shapes reviewers happened to find.

Each of these twelve fakes would look complete to a reviewer who only read the diff and not the tests — every one changes real code in the right direction, and every one still leaves a specific, describable input that reaches the forbidden outcome. The three-ASCII-hostname-shape sequence (prefix, substring-anywhere, suffix-without-boundary) is the reason this page does not claim two hostname anti-fake tests are enough: each shape is a distinct fake that the other two do not catch, and the `in`-substring fake's failure against all three, discovered only once the third test existed, is why every count above was rebuilt from scratch rather than assumed stable across additions. The trust-state fake is also the reason this page does not claim "unknown trust state fails closed" and "untrusted CA fails closed" are the same test: a check that never completed and a check that completed and failed are different facts about the world, and a suite that only exercises the first has not verified the second. The four dedicated version-check fakes are the same lesson again, one signal over, four times: "not below the oldest accepted version," "a member of the accepted set," "numerically within the accepted range," and "the same string after discarding incidental whitespace" are four different claims, not one claim stated four ways. The fifth and sixth version-check fakes — leading-zero normalization, and Unicode canonical-compatibility normalization — never got a dedicated test of their own at all, and that is the point rather than an oversight: by the time a sixth review round found the first of the two, the module had already stopped enumerating claims by hand and started asserting the underlying property (exact membership in a two-element set, nothing looser, no matter how the input is parsed or normalized) against a broad, adversarially-chosen input space — so the fifth claim was caught by a test written before anyone had stated it, and the sixth (the seventh review round's Unicode-normalization finding, which struck the hostname check too) needed only one new sweep case and one new dedicated hostname test, not a re-litigation of the whole strategy. A fake's failure count against the CURRENT suite, not the suite that existed when it was first isolated, is the only count worth trusting — and a suite that keeps needing that recomputation is a suite that would have benefited from a broader test sooner, which is exactly what happened here once it did.
...
Run both variants and, for the thirty-one tests that fail on `vulnerable`, name which row of the table above each one evidences (the seventeen parametrized near-miss-sweep cases all evidence the same row — treat them as one piece of evidence repeated seventeen ways, not seventeen independent findings):
