...
```mermaid
flowchart LR
  Run["pytest --impl vulnerable"] -->|"6 of 16 pass"| V1["C1 forbidden outcome: FAIL\n(cache leak reproduced)"]
  ...
  Fixed["pytest --impl fixed"] -->|"16 of 16 pass"| F1["C1, C2, C3 all hold,\nincluding every anti-fake test"]
...
`labs/2.2/2.2-request-path/README.md` documents seven constructed fakes verified against this suite before this module trusted it: a cache-only fix that leaves the header override open, matching `vulnerable/app.py`'s own check order — the API key's validity is decided first, then the header — so the malformed-credential case (an unregistered key with a company header) still passes; only the C2 forbidden-outcome test and its anti-fake pair catch it (two failures; 14 of 16 still pass); a header-only fix that leaves the cache keyed on path alone (caught symmetrically by C1's own forbidden-outcome and anti-fake tests, two failures; 14 of 16 still pass); a hostname check using `.startswith()` (caught by exactly one anti-fake test, the prefix one; 15 of 16 still pass); a hostname check using `in` (caught by both hostname anti-fake tests, since a substring match is a superset of a prefix match; 14 of 16 still pass); a trust-state check that reads "fails closed on an unrecognized state" as "checks only for `None`" — `if cert_trusted_ca is None: return False` in place of `if cert_trusted_ca is not True: return False` — which still fails closed on the unknown case but wrongly treats an explicitly-resolved `False` (a chain check that completed and failed) the same as `True` once hostname and version pass (15 of 16 still pass, and only the test built specifically to isolate that one signal catches it); a version check that reads "only 1.2 and 1.3 are accepted" as a lexicographic lower bound (`tls_version < "1.2"`) instead of exact set membership, which wrongly accepts any version string that merely sorts above "1.2" as text — such as "1.9" — without being a member of the accepted set at all (15 of 16 still pass, caught only by the test built to isolate that signal); and a version check that reads the same requirement as "add the next plausible version to the allow-list," hand-writing `{"1.2", "1.3", "1.4"}` in place of `_ACCEPTED_TLS_VERSIONS` itself — "1.4" was never an accepted version in this fixture, but nothing else in the suite ever sends it (15 of 16 still pass, caught only by the test built specifically because the six fakes before it never tried a version outside `{"1.1", "1.2", "1.3", "1.9"}`). Each of these seven fakes would look complete to a reviewer who only read the diff and not the tests — every one changes real code in the right direction, and every one still leaves a specific, describable input that reaches the forbidden outcome. The trust-state fake is also the reason this page does not claim "unknown trust state fails closed" and "untrusted CA fails closed" are the same test: a check that never completed and a check that completed and failed are different facts about the world, and a suite that only exercises the first has not verified the second. The two version-check fakes are the same lesson again, one signal over, from both directions: "not below the oldest accepted version" and "a member of the accepted set" are different claims, and so are "a version I guessed should be allowed" and "a version this fixture actually accepts" — a suite that only tests one direction of that gap has not verified the other.
