...
```mermaid
flowchart LR
  Run["pytest --impl vulnerable"] -->|"6 of 17 pass"| V1["C1 forbidden outcome: FAIL\n(cache leak reproduced)"]
  ...
  Fixed["pytest --impl fixed"] -->|"17 of 17 pass"| F1["C1, C2, C3 all hold,\nincluding every anti-fake test"]
...
`labs/2.2/2.2-request-path/README.md` documents eight constructed fakes verified against this suite before this module trusted it, each count checked by reconstructing the exact fake and running the real suite rather than incrementing an old number. A cache-only fix that leaves the header override open, matching `vulnerable/app.py`'s own check order — the API key's validity is decided first, then the header — so the malformed-credential case (an unregistered key with a company header) still passes; only the C2 forbidden-outcome test and its anti-fake pair catch it (two failures; 15 of 17 still pass). A header-only fix that leaves the cache keyed on path alone (caught symmetrically by C1's own forbidden-outcome and anti-fake tests, two failures; 15 of 17 still pass). A hostname check using `.startswith()` (caught by exactly one anti-fake test, the prefix one; 16 of 17 still pass). A hostname check using `in` (`expected_hostname in presented_hostname`) — caught by all three hostname anti-fake tests, not only two, because "origin.internal" is a substring of "evil-origin.internal" exactly as it is a substring of a prefix or middle-placed attacker string, so a check that only asks "does the expected name appear anywhere" cannot tell any of the three shapes apart (three failures; 14 of 17 still pass). A hostname check using `.endswith()` — the same mistake from the third boundary position, a name glued onto the end with no separator — caught by exactly one anti-fake test, the one built for this position (16 of 17 still pass). A trust-state check that reads "fails closed on an unrecognized state" as "checks only for `None`" — `if cert_trusted_ca is None: return False` in place of `if cert_trusted_ca is not True: return False` — which still fails closed on the unknown case but wrongly treats an explicitly-resolved `False` (a chain check that completed and failed) the same as `True` once hostname and version pass (16 of 17 still pass, and only the test built specifically to isolate that one signal catches it). A version check that reads "only 1.2 and 1.3 are accepted" as a lexicographic lower bound (`tls_version < "1.2"`) instead of exact set membership — caught by both version anti-fake tests, not only one: "1.9" sorts above "1.2" as text (the gap that test was built for), but so does "1.4" — `"1.4" < "1.2"` is `False` in Python, since `"4" > "2"` character-by-character — so this same fake also wrongly accepts the forward-compatible test's input for an unrelated reason (two failures; 15 of 17 still pass). And a version check that reads the same requirement as "add the next plausible version to the allow-list," hand-writing `{"1.2", "1.3", "1.4"}` in place of `_ACCEPTED_TLS_VERSIONS` itself — "1.4" was never an accepted version in this fixture, but nothing else in the suite ever sends it (16 of 17 still pass, caught only by the test built specifically because the fakes before it never tried a version outside `{"1.1", "1.2", "1.3", "1.9"}`). Each of these eight fakes would look complete to a reviewer who only read the diff and not the tests — every one changes real code in the right direction, and every one still leaves a specific, describable input that reaches the forbidden outcome. The three-hostname-shape sequence (prefix, substring-anywhere, suffix-without-boundary) is the reason this page does not claim two hostname anti-fake tests are enough: each shape is a distinct fake that the other two do not catch, and the `in`-substring fake's failure against all three, discovered only once the third test existed, is why every count above was rebuilt from scratch rather than assumed stable across additions. The trust-state fake is also the reason this page does not claim "unknown trust state fails closed" and "untrusted CA fails closed" are the same test: a check that never completed and a check that completed and failed are different facts about the world, and a suite that only exercises the first has not verified the second. The two version-check fakes are the same lesson again, one signal over, from both directions: "not below the oldest accepted version" and "a member of the accepted set" are different claims, and so are "a version I guessed should be allowed" and "a version this fixture actually accepts" — a suite that only tests one direction of that gap has not verified the other, and the lexicographic fake's failure against both, not just the one it was built against, is the same lesson the `in`-substring fake already taught about hostnames.
...
Run both variants and, for the eleven tests that fail on `vulnerable`, name which row of the table above each one evidences:
