...
```mermaid
flowchart LR
  Run["pytest --impl vulnerable"] -->|"5 of 13 pass"| V1["C1 forbidden outcome: FAIL\n(cache leak reproduced)"]
  ...
  Fixed["pytest --impl fixed"] -->|"13 of 13 pass"| F1["C1, C2, C3 all hold,\nincluding both anti-fake pairs"]
...
Each arrow's annotation names what the test actually inspected — a returned body, a status code, a rejected relay — not merely "ran successfully." A run that reports "5 passed" for `vulnerable` without checking *which* five would miss that the five passing tests are exactly the ones this module predicts should pass on the broken code (the shared normal case and the three cases that do not depend on either bug), while the eight that should fail do fail for the stated reason.
...
## Two anti-fake tests are not decoration

`labs/2.2/2.2-request-path/README.md` documents four constructed fakes verified against this suite before this module trusted it: a cache-only fix that leaves the header override open (caught by C2's forbidden-outcome test and its anti-fake pair, both fail; 11 of 13 still pass); a header-only fix that leaves the cache keyed on path alone (caught symmetrically by C1's pair); a hostname check using `.startswith()` (caught by exactly one anti-fake test, the prefix one); and a hostname check using `in` (caught by both hostname anti-fake tests, since a substring match is a superset of a prefix match). Each of these four fakes would look complete to a reviewer who only read the diff and not the tests — every one changes real code in the right direction, and every one still leaves a specific, describable input that reaches the forbidden outcome.
