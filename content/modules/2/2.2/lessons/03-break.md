...
The practice is not a website you attack. It is a tiny Python model of a shared cache. The failure is already in the object: the store keys only on path. The point is to see that the check treats that object as a **failed rule**, not as a performance nit.
