...
| C2 | 1 Property, 3 Break, 4 Build, 5 Verify | `test_forwarded_company_header_cannot_grant_a_different_companys_note` (forbidden outcome), `test_unregistered_api_key_with_a_company_header_is_denied` (malformed), `test_anti_fake_forwarded_header_with_fresh_never_elsewhere_used_values` | items.md #2, #4, #7 |
| C3 | 1 Property, 4 Build, 5 Verify, 6 Operate | `test_matching_hostname_trusted_ca_current_version_is_trustworthy`, `test_hostname_mismatch_with_trusted_ca_is_rejected` (forbidden outcome), `test_tls_1_2_boundary_is_still_accepted_1_1_is_not` (boundary), `test_unknown_trust_state_fails_closed` and `test_explicitly_untrusted_ca_is_rejected_even_with_matching_hostname_and_version` (malformed/anti-fake pair), nine anti-fake tests (hostname prefix, hostname substring-anywhere, hostname suffix-without-boundary, hostname Unicode-confusable, version-string-sorts-high, forward-compatible-allow-list, version-string-parses-in-range, version-string-incidental-whitespace, a seventeen-case version-string near-miss sweep including a Unicode-confusable case) | items.md #3, #7, #8 |
...
- `hop_is_trustworthy`'s hostname and version-string comparisons are exact code-point equality, and the lab's anti-fake tests confirm this holds even against a plausible Unicode-normalizing implementation (a fullwidth-digit version string, a fullwidth-letter hostname). What is NOT covered: real internationalized domain names (IDNA/Punycode), homograph attacks using visually similar but code-point-distinct scripts in a genuinely registered hostname, and any normalization a real certificate-validation library might legitimately perform before this predicate ever sees a hostname string. This module's claim is narrower and code-level — the check itself must not normalize away a genuine mismatch — not a claim that this fixture models IDN security generally.
