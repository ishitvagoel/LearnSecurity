# 9.1-LO-01 — Verification requirements and traceability: property vs mechanism
...
What must remain true of **SecureCollab** (or the elective system) regarding **Verification requirements and traceability** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For Verification requirements and traceability |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **Verification requirements and traceability**. Name what the app must still enforce.
...
Complete the associated lab under `labs/9.1/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **verification** property were false.
...
- Verification requirements and traceability is a Top 10 memorization exercise
