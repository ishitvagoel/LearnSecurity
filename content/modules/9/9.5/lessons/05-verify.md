...
A test that only counts passing tests can pass while `{retest: None}` still closes. Ask whether a finding closed without a retest still counts as a passing control. The broken files must fail that. The repaired files must pass it.
