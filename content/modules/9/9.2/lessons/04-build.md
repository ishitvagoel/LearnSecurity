...
The smallest restore for notes-app merge gating is: `x = eval(user)` → not approved. Fail closed: unknown dynamic execution denies in a real review even if this fixture’s substring misses it. Do not treat the denylist as the whole avoid-eval rule. Do not fail open because continuous integration formatted the file.
...
Industry lists want you to avoid eval. This pytest is that sentence for the lab string.
