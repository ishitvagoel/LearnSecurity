# 9.3-LO-03 — Local fixture illustrating 9.3 failure (authorized only)
...
What must remain true of **SecureCollab** (or the elective system) regarding **Security-focused tests** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For Security-focused tests |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **Security-focused tests**. Name what the app must still enforce.
...
Complete the associated lab under `labs/9.3/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **focused** property were false.
...
- Security-focused tests is a Top 10 memorization exercise
