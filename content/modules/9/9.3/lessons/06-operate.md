...
Line coverage percent does not make a 200-only isolation row a security test. Detection must observe **200-only is not a security test**, not percent covered. Note bodies from a failed isolation case next to a 200-only suite are a log-line leak (3.1).
