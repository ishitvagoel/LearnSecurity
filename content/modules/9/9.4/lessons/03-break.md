...
The practice is not a public repo you scan. It is a tiny Python `ship_ok` that takes findings and a map. The failure is already in the function: it returns true for every pair. The point is to see that the check treats that true as a **failed rule**, not as a green tile.
