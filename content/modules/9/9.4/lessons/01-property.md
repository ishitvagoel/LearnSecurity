# 9.4-LO-01 — Automated analysis and tool orchestration: property vs mechanism
...
What must remain true of **SecureCollab** (or the elective system) regarding **Automated analysis and tool orchestration** when an attacker with stated capabilities acts, a component fails, or a human follows a stressful recovery path?
...
| Slice | For Automated analysis and tool orchestration |
|---|---|
...
FastAPI, Next.js, PostgreSQL, or Android “secure defaults” are not the application guarantee for **Automated analysis and tool orchestration**. Name what the app must still enforce.
...
Complete the associated lab under `labs/9.4/` if a labSpec exists. Observe the forbidden outcome on `vulnerable/`. Do not target non-lab systems. Do not copy weaponized payloads into notes.

Safe task: write one testable sentence that would fail if the **automated** property were false.
...
- Automated analysis and tool orchestration is a Top 10 memorization exercise
