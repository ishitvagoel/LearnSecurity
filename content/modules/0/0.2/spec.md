...
| Grant a tooling-bridge skip only on the diagnostic's own observed gap evidence, never on a job title, vendor certification, or LMS badge claim | C2 | `lessons/02-model.md` §A credential is not a diagnostic | `lessons/02-model.md` evidence-source table | `labs/0.2/0.2-bridge` `tooling_bridge_required` tests | items.md #2, #3, #5 | `lessons/07-transfer.md` vendor cert vs. threat-model review |
...
| Treat the NICE Secure Systems Development work role as informative placement vocabulary, not as Gate 1 evidence or a substitute syllabus | C4 | `lessons/02-model.md` §Vocabulary is not a syllabus | `lessons/02-model.md` work-role-vs-cell table | `lessons/08-review.md` review exercise | items.md #6 | `lessons/07-transfer.md` |
