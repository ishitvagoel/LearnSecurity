...
## 2. Discrimination — property vs. mechanism for an accessibility fix

Four statements about the recovery-confirm control:

**A.** "We use an accessible component library, so the control is accessible."
**B.** "Our WCAG scanner reports zero violations on this page."
**C.** "The confirm control declares `name='Confirm account recovery'`, `keyboard=True`, `mouse_only=False`, and `is_usable_accessible` returns `True` for it."
**D.** "The confirm control must be operable by keyboard, have a name a screen reader can speak, and not rely on color alone as the only cue; if any one of these is missing, the control has failed as a security control, not merely as a UI defect."

Identify which statement states the property (the rule itself) and which state mechanisms or proxies for it, and explain what each mechanism claim would still need to show before it could stand in for the property.

**Claim assessed:** C2, C3 · **Outcome:** Treat security friction and inaccessibility as security outcomes

## 3. Diagnosis — the checker's quiet gap
...
## 4. Diagnosis — reading a "fixed" diff
...
## 5. Diagnosis — friction theater vs. a convenience hole
...
## 7. Design — actors and the support-read-aloud row
...
## 8. Transfer — the clinic and bank sketches
...
- [ ] Transfer answer (item 8) naming which claims change and which do not
- [ ] Operate signal (item 9) that carries no recovery codes or note bodies
