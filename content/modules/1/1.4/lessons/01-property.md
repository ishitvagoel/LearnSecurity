...
Industry lists name families like govern, detect, respond, recover — NIST CSF 2.0's own top-level functions are exactly these words. They are a useful vocabulary for the bottom half of the loop, but naming a function is not the same claim as proving this sentence about the notes app's recovery journey. A maturity score is not leftover risk, and buying a second factor is not the rule; both are inputs a designer might use on the way to a checkable claim, not substitutes for one.
